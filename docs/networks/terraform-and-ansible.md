...
script](https://github.com/MagHErmit/tendermint/blob/v0.34.x/networks/remote/integration.sh)
...
```sh
cd $GOPATH/src/github.com/MagHErmit/tendermint/networks/remote/terraform
```
...
directory](https://github.com/MagHErmit/tendermint/tree/v0.34.x/networks/remote/ansible)
run ansible roles to configure the sentry node architecture. You must
switch to this directory to run ansible
(`cd $GOPATH/src/github.com/MagHErmit/tendermint/networks/remote/ansible`).
...
```sh
ansible-playbook -i inventory/digital_ocean.py -l sentrynet config.yml -e BINARY=$GOPATH/src/github.com/MagHErmit/tendermint/build/tendermint -e CONFIGDIR=$GOPATH/src/github.com/MagHErmit/tendermint/networks/remote/ansible/mytestnet
```
